...
//             const prod = await new Post(product);
//             const savedPost = await prod.save(product);
//             savedPost.populate('createdBy').populate('tags').execPopulate().then(async (sd) => {
//                 console.log(sd);
...
//             await Post.findByIdAndUpdate(product._id, product, { new: true }, (err, res) => {
//                 if (err) {
//                     return reject(err)
//                 }
...
//             Post.find({ 'createdBy': userId, status: status ? status : { $ne: null } }).populate('createdBy').populate('tags').exec((err, res) => {
...
//             Post.findById(productId).populate('createdBy').populate('tags').exec(async (err, res) => {
...
//             Post.deleteOne({ _id: productId }, ((err, res) => {
